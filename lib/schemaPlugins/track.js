...
module.exports = function track () {
	...
	_.forEach(options, function (value, key) {
		...
		if (_.has(defaultOptions, key)) {
				...
				switch (key) {
					...
					case 'updatedFrom':
						fields[fieldName] = {
							...
							noedit: true,
							collapse: true,
							// use delegated options list
							assign: true,
							default: updateFromOptions.cms.value,
							options: 'updatedFrom',
						};
						break;
				}
			...
			throw new Error('Invalid List "track" option for ' + list.key + '\n'
				+ 'valid field options are "createdAt", "createdBy", "updatedAt", "updatedFrom",  and "updatedBy".');
		}
	});
	...
	list.schema.pre('save', function (next) {
		...
		if (!this.updatedFrom) {
			// default update by API side
			this.updatedFrom = updateFromOptions.api.value;
		}
