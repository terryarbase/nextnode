...
function getOptions () {
	var ops = {
		/*
		** Copy all of properties in Options, instead of spreading each of properties manually
		** Terry Chan
		** 25/09/2019
		*/
		...this.options,
	...
	_.forEach(this.uiElements, function (el) {
		...
		switch (el.type) {
			...
			case 'field':
				...
				const fields = _.get(el, 'field.fields');
				// nested hidden fields
				if (fields) {
					_.forEach(fields, (f, k) => {
						if (f.options.hidden) {
							_.unset(ops, `fields.${el.field.path}.fields.${k}`);
						}
					});
				}
