...
function transformFieldValue (field, item, options) {
	...
	return field.format(item, options);
}

function convertMultilingual(field, item, { langd }) {
	...
	if (options.fields) {
		...
		options.fields.forEach(function (path) {
			...
			if (!field) {
				...
				data[path] = convertMultilingual(field, item.get(path), options);
				return;
			}
			if (field.type !== 'relationship' || !options.expandRelationshipFields) {
				// use the transformFieldValue function to get the data
				// const newData = convertMultilingual(field, item.get(path), options);
