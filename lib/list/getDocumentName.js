...
function getDocumentName (doc, escape, options) {
	...
	if (this.nameField) {
		name = this.nameField.format(doc);
		path = this.nameField.path;
	}
	if (this.fields[path].options.multilingual) {
		const { lang, defaultLang } = options;
		const isMultilingualFormat = this.isMultilingualFormat(name);
