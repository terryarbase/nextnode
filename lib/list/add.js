...
function add () {
	...
	const self = this;
	const keystone = self.keystone;
	var atLeastMultilingual = false;
	...
	var addField = function (path, options) {
		...
		** fired when the localization is on and the multilingual is on from the list config
		*/
		const { type: { name }, multilingual } = options;
		// auto assign
		if (keystone.get('localization') && self.options.multilingual) {
			// auto assign multilingual flag to all of preserve Types if no single lingual provided
			if (self.isMultilingual(name)
				&& path !== 'delegated') {
				// set to false multilingual flag manually
				if (_.isBoolean(multilingual) && !multilingual) {
					options.multilingual = false;
				} else {
					atLeastMultilingual = options.multilingual = true;
				}
			} else if (self.isBlockingMultilingual(name) && multilingual)  {
				throw new Error(
					`Invalid multilingual config for schema ${self.key} at path ${path} with Types: "${name}". Please remove the "multilingual:true" config first.`
				);
			} else {
				options.multilingual = false;
			}
	...
	// console.log(">>>>>>>>>>> ", this.key, atLeastMultilingual);
	// if (!atLeastMultilingual) {
	// 	// turn off multilingual
	// 	this.set('multilingual', atLeastMultilingual);
	// }
	return this;
}
