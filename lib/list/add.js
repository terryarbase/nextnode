var _ = require('lodash');
var utils = require('keystone-utils');

/**
 * Adds one or more fields to the List
 * Based on Mongoose's Schema.add
 */
function add () {
	var args = Array.prototype.slice.call(arguments);
	const self = this;
	const keystone = self.keystone;
	const types = self.keystone.Field.Types;
	const localizationType = [
		types.Text.name,
        types.Textarea.name,
        types.TextArray.name,
        types.Html.name,
        types.Url.name,
        types.Name.name,
	];
	var add = function (obj, prefix) {
		prefix = prefix || '';
		var keys = Object.keys(obj);
		for (var i = 0; i < keys.length; ++i) {
			var key = keys[i];
			if (!obj[key]) {
				throw new Error(
					'Invalid value for schema path `' + prefix + key + '` in `' + this.key + '`.\n'
					+ 'Did you misspell the field type?\n'
				);
			}
			if (utils.isObject(obj[key]) && (!obj[key].constructor || obj[key].constructor.name === 'Object') && (!obj[key].type || obj[key].type.type)) {
				if (Object.keys(obj[key]).length) {
					// nested object, e.g. { last: { name: String }}
					// matches logic in mongoose/Schema:add
					this.schema.nested[prefix + key] = true;
					add(obj[key], prefix + key + '.');
				} else {
					addField(prefix + key, obj[key]); // mixed type field
				}
			} else {
				addField(prefix + key, obj[key]);
			}
		}
	}.bind(this);

	var addField = function (path, options) {
		if (this.isReserved(path)) {
			throw new Error('Path ' + path + ' on list ' + this.key + ' is a reserved path');
		}
		var newOptions = { ...options };
		/*
		** Localization setting
		** fired when the localization is on and the multilingual is on from the list config
		*/
		if (keystone.get('localization') && self.options.multilingual) {
			const { multilingual, restrictDelegated, type: { name } } = options;
			// auto assign multilingual flag to all of preserve Types
			if (_.includes(localizationType, name)) {
				newOptions = {
					...newOptions,
					...{
						multilingual: true,
					},
				};
			// // ignore customized multilingual fields if restrictDelegated is on
			// // this mean the field cannot be 
			// } else if (multilingual && !restrictDelegated) {
			// 	options = {
			// 		...options,
			// 		...{
			// 			multilingual: false,
			// 		},
			// 	};
			}
		}

		this.uiElements.push({
			type: 'field',
			field: this.field(path, newOptions),
		});
	}.bind(this);

	_.forEach(args, function (def) {
		self.schemaFields.push(def);
		if (typeof def === 'string') {
			if (def === '>>>') {
				self.uiElements.push({
					type: 'indent',
				});
			} else if (def === '<<<') {
				self.uiElements.push({
					type: 'outdent',
				});
			} else {
				self.uiElements.push({
					type: 'heading',
					heading: def,
					options: {},
				});
			}
		} else {
			if (def.heading && typeof def.heading === 'string') {
				self.uiElements.push({
					type: 'heading',
					heading: def.heading,
					options: def,
				});
			} else {
				add(def);
			}
		}
	});

	return this;
}

module.exports = add;
