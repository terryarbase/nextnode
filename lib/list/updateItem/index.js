	...
	function doFeildsValidation (field, newData, info={}, callback) {
		field.validateInput(newData, function(valid, detail) {
			if (!valid) {
			    ...
			    field.validateRequiredInput(item, newData, function(valid, detail) {
			        if (!valid) {
	...
	function doFieldValidation (field, done) {
		...
		if (restricted) {
			...
			doFeildsValidation(field, defaultLangData, { lang: options.defaultLang }, function() {
				...
				const tasks = [];
				// rest validation for inputed value
				// use es5 aysnc parallel instead of await promise to maintain the style consistent 
				_.forOwn(restLangData, (current, lang) => {
					...
					if (
						(!_.isNil(current) && !_.isNull(current) && !!String(current)) || 
						lang === options.lang
					) {
			...
			doFeildsValidation(field, data, null, done);
		}
	}
				...
				lang.forEach(function(lang) {
					tasks.push(function(cb) {
						...
						field.updateItem.apply(field, [item, {
							...
							// '__fileQueue': fileValues,
							'__subPath': lang,
						}, options.files, cb]);
						// , function(err, result) {
						// 	if (!err) {
						// 		fileValues[lang] = result;
						// 	}
						// 	cb(err);
						// }]);
						// field.updateItem(item, {
						// 	[field.path]: data[field.path][lang],
						// }, options.files, cb);
					});
				});
