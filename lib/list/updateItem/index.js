...
const updateItem = function(item, originalData, options, callback) {
	...
	*/
	...
	fields = listToArray(fields).reduce(function (a, field) {
		// TODO: Check that field is an instance of Field
		if (field) {
			const newField = (typeof field === 'string') ? self.fields[field] : field;
			if (newField && newField.options) {
				newField.setOptions({
					// the field is multilingual and the keystone is opened for localization
					isMultilingual: newField.options.multilingual && options.isMultilingual,
	...
	if (fields.indexOf(undefined) >= 0) {
		return callback({
			...
			message: options.__('msg_invalid_path_update', {
	...
	function doFeildsValidation (field, newData, info={}, callback) {
		field.validateInput(newData, function(valid, detail) {
			if (!valid) {
			    addValidationError({ ...options, field, ...info }, validationErrors, field, 'invalid', detail);
			    callback();
			} else if ((field.required || requiredFieldPaths[field.path]) &&
			    (!field.dependsOn || evalDependsOn(field.dependsOn, data))) {
				let newItem = Object.assign(Object.create(item), item);
				// console.log(item, item[field.path]);
				if (field.options.isMultilingual) {
					if (info.lang && item[field.path] && item[field.path][info.lang]) {
						newItem[field.path] = item[field.path][info.lang];
					}
				} else {
					if (item[field.path]) {
						newItem[field.path] = item[field.path];
					}
				}
				// console.log('> field: ', field.path, ', ' , newItem);
			    field.validateRequiredInput(
			    	newItem,
			    	newData,
			    function(valid, detail) {
			    	// console.log(field.path, '>>> ', newData, valid);
			        if (!valid) {
			            addValidationError({ ...options, field, ...info }, validationErrors, field, 'required', detail);
			        }
			        callback();
			    });
			} else {
	...
	function doFieldValidation (field, done) {
		...
		if (restricted) {
			addValidationError({ ...options, field }, validationErrors, field, 'forbidden', null);
			...
			doFeildsValidation(field, defaultLangData, { lang: options.uiLang }, function() {
			...
			doFeildsValidation(field, data, { lang: options.uiLang }, done);
		}
	}
	...
	function doFieldUpdate (field, done) {
		...
		var callback = function (err) {
			...
			if (err) {
				addFieldUpdateError({ ...options, field }, updateErrors, field, err);
			}
			done();
		};
				...
				if (err.code === 11000) {
					...
					if (indexConstraintError) {
						...
						return callback({
							...
							message: options.__('msg_duplication_value', {
								field: probableFieldPath,
								value: indexConstraintError[2],
							}),
						});
					}
				}
