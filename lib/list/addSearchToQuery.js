...
function getNameFilter (field, searchString, { isMultilingual, langd }) {
	...
	if (isMultilingual && langd) {
		query = {
			$or: [
				query,
				{
					[`${field.path}.${langd}${field.paths.firstname}`]: nameSearchRegExp,
				},
				{
					[`${field.path}.${langd}${field.paths.lastname}`]: nameSearchRegExp,
				}
			],
		};
	}
...
function getStringFilter (path, searchRegExp, { isMultilingual, langd }) {
	var filter = {};
...
function addSearchToQuery (searchString, options) {
	// const { langd } = options;
	...
	if (this.options.searchUsesTextIndex) {
		...
		searchFilters = this.searchFields.map(function (i) {
			const isMultilingual = i.field && i.field.options.multilingual;
			const fieldOptions = {
				...options,
				isMultilingual,
			};
			if (i.field && i.field.type === 'name') {
				return getNameFilter(i.field, searchString, fieldOptions);
			} else {
				return getStringFilter(i.path, searchRegExp, fieldOptions);
			}
		}, this);
