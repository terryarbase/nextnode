...
async function createDelegatedAdmin() {
    ...
    try {
        ...
        if (delegatedPermission) {
            ...
            updateLatestListPermission(nextNode, permissionList);
            ...
            _forEach(permissionSchema, (listSchema, listName) => {
                permissionData[listName] = {};
                _forEach(listSchema.fields, (fieldSchema, fieldName) => {
        ...
        if (delegatedUser) {
            ...
            //     _forEach(countryRelationship, field => {
            //         userData = {
            //             ...userData,
...
async function updateLatestListPermission(nextNode, permissionList) {
    const allLists = nextNode.lists;
    const allListKeys = _keys(allLists);
    const permissions = await permissionList.model.find();

    _forEach(permissions, p => {
        // pick up exist list
        let updated = _pickBy(p.toObject(), (value, key) => _includes(allListKeys, key));

        // get the missing list
        const newList = _difference(allListKeys, _keys(updated));

        // add new list permission back to record
        _forEach(newList, list => {
            const listPermission = {};
            _forOwn(allLists[list].fields, (schema, field) => {
                listPermission[field] = 2;
                updated = {
                    ...updated,
                    [list]: listPermission,
                }
