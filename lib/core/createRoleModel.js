...
const createDelegatedRole = async (node, model, permissions) => {
    ...
    try {
        ...
        if (!newRole) {
            ...
            console.log('> Create a delegated account role: ', newRole);
        } else {
            console.log('> Delegated account role, and no need to create');
        }
    } catch (err) {
        console.log('Cannot create a delegated account role.');
        console.log(err);
        process.exit(1);
    }
};
